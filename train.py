...
def sampling_per_class(per_class, ground_truth):
    train = {}
    test = {}
    labels_loc = {}
    m = max(ground_truth)
...
if Dataset == 'CHIKUSEI':
    ...
    data_hsi = uPavia['chikusei'][:].transpose()
    gt_uPavia = sio.loadmat('datasets/Chikusei_gt.mat')
    gt_hsi = gt_uPavia['GT']
...
if Dataset == 'HS':
    ...
    gt_uPavia = sio.loadmat('datasets/Houston_gt.mat')
...
for index_iter in range(ITER):
    ...
    with tf.Session(config=config, graph=graph) as sess:
        ...
        batches_data_test, batches_labels_test, batch_count_test = into_batch(x_test, y_test, batch_size*4, shuffle=False)
    ...
    confusion_matrix_aucn = metrics.confusion_matrix(pred_aucn, gt_test[:-VAL_SIZE])
